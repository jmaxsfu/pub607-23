...
If that's what's in an ebook, then is makes a certain amount of sense to *develop them as if they were websites:*  as collections of HTML and CSS -- rather than treating them as weird exports out of the print production process in InDesign (an approach which is all too common). Because what's inside an ebook is a set of *text files*, it helps to treat them as such if we want to truly understand what's going on in there, and if we want to build them with precision.
...
We'll do our conversions with Pandoc. Pandoc has one zillion options -- and remember, there's a [Pandoc tutorial](Pandoc.md) -- but its simplest possible invocation will transform our markdown file into HTML and let us look at it in a web browser
