...
In this course we are going to centre our discussion on a tool called **Audacity**, which is a robust, venerable, open-source audio editor. It's been around for years, does all the important things, and is kind of ugly. If you need something different, you are encouraged to branch out to prettier, fancier, less open-source tools like GarageBand, Adobe Audition, Logic, Hindenburg, and so on. A noteworthy alternative is **Reaper**, which claims to have an accessibility layer for the visually impaired.

Despite differences in user interface design and the fine details of functionality, all of these tools work more or less the same way.

The fundamental metaphor is kind of a *magical tape recorder*. Like a tape recorder, the basic unit is the **track** -- that is, one audio source -- but in the software you can both *hear* and *see* it, via a depiction of the actual waveform.

You can have multiple tracks, either recorded simultaneously (as in "multitrack recording") or layered one at a time. If you were recording music, you would almost certainly record multiple tracks; for an audiobook with a single narrator, you might need only one or two.
...
2. Plug headphones into the Zoom recorder so you can "monitor" what you're recording. This is **important** -- if you can head what you're getting, it will save you hours of lost time with bad "takes."

3. Find a **quiet** room. You need to do this with headphones on.

4. Say "test, 1, 2, 3" while fiddling with the level controls on the Zoom to ensure that you are getting a strong signal that does not "clip" or overload, especially on the letter "P".

5. Press record and say your piece. End the recording, save the recording with a name you'll remember in step 7 below.

6. Plug the Zoom recorder into your laptop with a USB cable. Plug your headphones into your laptop too.

7. Load the recoding onto your computer. Open up Audacity. Import the recording into Audacity. Play it back. Does it sound alright? If not, go back to Step 2.
...
9. Stay up late into the night playing with digital effects. Listen carefully for Satanic messages hidden in the audio when you play it backwards (sorry, 80s reference; alternatively, listen for "Paul is dead," a 60s reference).

10. Decide that minimal effects (or none at all) is probably the best option. Hopefully you have not stayed up all night coming to this conclusion.

11. Export the recording as an MP3, and post it on your Github repo.

*There are many variations on this process. You can also record direct to your computer by using the Zoom as an "audio interface" instead of a recorder. You can also try different microphones, and different microphone placement.*
...
A **condenser mic** works differently: a small electric current flows through the mic, and the sound waves from your voice wiggle the "condenser," which affects the electric signal. Condenser mics are more sensitive at a distance, and therefore better for capturing audio from a farther and/or larger source (like a piano, or a choir) and more ambient sound from the room. You do not "eat" these mics... probably more like 20–50cm away.
